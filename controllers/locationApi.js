...
module.exports = function (express) {
  ...
  api.post("/createcountry", async (req, res) => {
    try {
      ...
      if (data.status) {
        return res.status(constants.STATUS_201).send({ statusCode: constants.STATUS_201, message: constants.STATUS_MSG_201, data: data.data });
      } else {
  ...
  api.get("/getcountries", async (req, res) => {
    try {
      ...
      if (data.status) {
        res.status(constants.STATUS_200).send({ statusCode: constants.STATUS_200, message: constants.STATUS_MSG_200, data: data.data });
      } else {
        return res.status(constants.STATUS_401).send({ statusCode: constants.STATUS_401, message: constants.STATUS_MSG_401, status: constants.STATUS_FALSE });
      }
    } catch (e) {
  ...
  api.post("/updatecountry/:id", async (req, res) => {
    try {
      ...
      if (data.status) {
        return res.status(constants.STATUS_200).send({
          data:data.data,
